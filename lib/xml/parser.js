...
function parseTasks(xml) {
  ...
  return parseXMLString(xml).then((result) => {
    ...
    parsed.forEach((event) => {
      ...
      const dtstart = vtodo.getFirstPropertyValue("dtstart"); //when task begin
      var dtend = vtodo.getFirstPropertyValue("due"); //when task should end
      if (dtend == null){ //If no end, maybe we have a duration
        var duration = vtodo.getFirstPropertyValue("duration");
        if (duration != null){
          //We have duration, so we know end
          dtend = moment(dtstart).add(moment.duration(duration));
        }
      }
      const dtcompleted = vtodo.getFirstPropertyValue("completed");

      let taskData = {
        title:vtodo.getFirstPropertyValue("summary"), //title
        uid: vtodo.getFirstPropertyValue("uid"), //id
        description: vtodo.getFirstPropertyValue("description"), //description
        start: dtstart ? new Date(dtstart).toISOString() : null, //when task begin null = now
        createdAt: new Date(vtodo.getFirstPropertyValue("created")).toISOString(), //creation date
        lastModified:new Date(vtodo.getFirstPropertyValue("last-modified")).toISOString(), //modification date
        status : vtodo.getFirstPropertyValue("status"), //text status
        completed : dtcompleted ? new Date(dtcompleted).toISOString() : null,
        percent : vtodo.getFirstPropertyValue("percent") || vtodo.getFirstPropertyValue("percent-complete") || 0,
        due : dtend ? new Date(dtend).toISOString() : null //when task end null = no end required
      }
...
exports.parseAddTask = parseAddTask;
