...
var Receiver = function (config) {
    if (typeof config === 'undefined') throw new Error('undefined is not a valid configuration object.')
...
Receiver.prototype.getCalendarWithCtag = function () {
    return this.doRequest(xml.calendar_ctag, 'PROPFIND', 0).then(xmlParser.parseCalendarMultistatus);
};

Receiver.prototype.getEventsWithEtag = function () {
    return this.doRequest(xml.events_etag, 'REPORT', 1).then(xmlParser.parseEventsMultistatus);
};
...
Receiver.prototype.getEvents = function (events) {
    ...
    return this.doRequest(multiget, 'REPORT', 1).then(xmlParser.parseEvents);
};

Receiver.prototype.getAllEvents = function () {
    return this.doRequest(xml.calendar_data, 'REPORT', 1).then(xmlParser.parseEvents);
};

Receiver.prototype.getEventsFromToday = function(startDate, endDate) {
    ...
    var request = ejs.compile(xml.byTime)({startDate: startDate, endDate: endDate});
    return this.doRequest(request, 'REPORT', 1).then(xmlParser.parseEvents);
};
